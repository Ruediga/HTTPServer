...
bool Socket::accept(Socket &new_socket, std::string &connectionIP) const
{
    ...
    // [TODO] fix
    //inet_ntop(AF_INET, &(m_addr.sin_addr), ipStr, INET_ADDRSTRLEN);
...
bool Socket::connect(const std::string host, const int port)
{
    ...
    // if addr is numerical ip addr [TODO]
...
int Socket::recv(std::string &s) const
{
    ...
    {
        std::cout << "Socket::recv threw an error - data couldn't be received: " << WSAGetLastError() << std::endl;
        return -1;
    }
}
